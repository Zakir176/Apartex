<template>
  <div class="min-h-screen bg-white text-slate-800 selection:bg-accent selection:text-white">

    <!-- HERO SECTION -->
    <header class="bg-white border-b border-surface-border relative pt-16 pb-20">
      <div class="max-w-content mx-auto px-4 sm:px-6 relative z-10">
        
        <!-- Live Trust Badge -->
        <div class="flex justify-center mb-6">
          <div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-md bg-accent-light border border-accent/20 text-xs font-medium text-accent">
        ...
        <div class="text-center max-w-4xl mx-auto mb-10">
          <h1 class="text-4xl sm:text-5xl md:text-6xl font-bold text-gray-900 tracking-tight leading-tight mb-6 animate-fade-up">
            Find Your Stay<br class="hidden sm:block" />
            <span class="text-accent">Across Zambia</span>
          </h1>
          <p class="text-base sm:text-lg text-gray-500 font-normal max-w-xl mx-auto leading-relaxed animate-fade-up-delay-1">
        ...
        <div class="max-w-5xl mx-auto bg-white rounded-xl border border-surface-border shadow-lg p-3 sm:p-5 relative z-20 animate-fade-up-delay-2">
          <!-- Search Type Tabs -->
          <div class="flex items-center gap-2 mb-4 border-b border-surface-border pb-3 overflow-x-auto no-scrollbar">
            <button
              ...
              class="px-4 py-2 rounded-lg text-xs sm:text-sm font-semibold transition-all flex items-center gap-2 whitespace-nowrap cursor-pointer"
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 items-center">
            <!-- Location -->
            <div class="flex flex-col px-4 py-2.5 bg-slate-50 hover:bg-white rounded-lg border border-slate-200 transition-all focus-within:ring-2 focus-within:ring-accent focus-within:border-accent">
              <label class="text-[10px] font-medium text-slate-400 uppercase tracking-wider mb-1 flex items-center gap-1">
                <i class="pi pi-map-marker text-accent"></i> Location
              </label>
              <select v-model="selectedCity" class="bg-transparent text-sm font-semibold text-slate-800 outline-none cursor-pointer border-none p-0 focus:ring-0">
            ...
            <div class="flex flex-col px-4 py-2.5 bg-slate-50 hover:bg-white rounded-lg border border-slate-200 transition-all focus-within:ring-2 focus-within:ring-accent focus-within:border-accent">
              ...
              <input 
                ...
                v-model="checkInDate" 
                :min="todayDate" 
                class="bg-transparent text-sm font-semibold text-slate-800 outline-none cursor-pointer border-none p-0 focus:ring-0" 
              />
            </div>
            ...
            <div class="flex flex-col px-4 py-2.5 bg-slate-50 hover:bg-white rounded-lg border border-slate-200 transition-all focus-within:ring-2 focus-within:ring-accent focus-within:border-accent">
              ...
              <select v-model="guestCount" class="bg-transparent text-sm font-semibold text-slate-800 outline-none cursor-pointer border-none p-0 focus:ring-0">
            ...
            <button 
              @click="triggerSearch" 
              class="w-full h-full min-h-[52px] bg-accent hover:bg-accent-hover text-white font-semibold text-sm rounded-lg px-6 py-3.5 flex items-center justify-center gap-2 shadow-accent transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] cursor-pointer"
            >
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-4xl mx-auto mt-12 text-center animate-fade-up-delay-3">
          <div class="bg-white/60 backdrop-blur-md p-4 rounded-xl border border-surface-border">
            <p class="text-2xl sm:text-3xl font-semibold text-navy">1,200+</p>
            <p class="text-xs text-slate-500 font-semibold mt-1">Verified Stays</p>
          </div>
          <div class="bg-white/60 backdrop-blur-md p-4 rounded-xl border border-surface-border">
            <p class="text-2xl sm:text-3xl font-semibold text-accent">4.9 ★</p>
            <p class="text-xs text-slate-500 font-semibold mt-1">Average Guest Rating</p>
          </div>
          <div class="bg-white/60 backdrop-blur-md p-4 rounded-xl border border-surface-border">
            <p class="text-2xl sm:text-3xl font-semibold text-navy">$0</p>
            <p class="text-xs text-slate-500 font-semibold mt-1">Hidden Booking Fees</p>
          </div>
          <div class="bg-white/60 backdrop-blur-md p-4 rounded-xl border border-surface-border">
            <p class="text-2xl sm:text-3xl font-semibold text-emerald-600">24/7</p>
    ...
    <section class="max-w-content mx-auto px-4 sm:px-6 py-20">
      <div class="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4 reveal-up">
        ...
        <div class="flex items-center gap-1.5 overflow-x-auto pb-2 no-scrollbar">
          <button
            ...
            class="px-4 py-2 rounded-lg text-xs font-semibold transition-all whitespace-nowrap cursor-pointer"
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 reveal-up delay-100">
        <div
          ...
          class="group relative rounded-xl overflow-hidden cursor-pointer shadow-sm hover:shadow-lg transition-all duration-500 aspect-[4/5] flex flex-col justify-end"
        ...
        >
          ...
          <div class="absolute top-4 left-4 right-4 flex justify-between items-center z-10">
            <span class="bg-white/90 backdrop-blur-md text-slate-800 text-xs font-semibold px-3 py-1 rounded-md shadow-sm">
              {{ destination.count }} Properties
            </span>
            <span class="bg-navy/80 backdrop-blur-md text-white text-[11px] font-semibold px-2.5 py-1 rounded-md flex items-center gap-1">
          ...
          <div class="relative z-10 p-6">
            <span class="text-xs font-semibold text-accent uppercase tracking-widest block mb-1">{{ destination.tagline }}</span>
            <h3 class="text-2xl font-semibold text-white leading-tight mb-2 group-hover:text-amber-300 transition-colors">
              {{ destination.name }}
            </h3>
            <p class="text-white/70 text-xs line-clamp-2 leading-relaxed mb-4">
              {{ destination.description }}
            </p>
            <div class="flex items-center text-white font-semibold text-xs group-hover:translate-x-2 transition-transform duration-300">
    ...
    <section class="bg-white border-y border-surface-border py-20">
      <div class="max-w-content mx-auto px-4 sm:px-6">
        <div class="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4 reveal-up">
          ...
          <div class="flex items-center gap-2 overflow-x-auto pb-2 no-scrollbar">
            <button
              ...
              class="px-4 py-2 rounded-lg text-xs font-semibold transition-all cursor-pointer whitespace-nowrap"
              :class="activeCategory === category ? 'bg-navy text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'"
            >
              {{ category }}
            </button>
            <router-link to="/apartments" class="text-xs font-semibold text-accent hover:text-accent-hover ml-2 whitespace-nowrap no-underline">
        ...
        <div v-if="apartmentsStore.loading" class="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div v-for="i in 3" :key="i" class="bg-slate-50 rounded-xl border border-surface-border overflow-hidden">
        ...
        <div v-else-if="displayApartments.length > 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 reveal-up delay-200">
          <div 
            ...
            :key="apartment.id"
            class="bg-white rounded-xl border border-surface-border shadow-card hover:shadow-card-hover transition-all duration-300 hover:-translate-y-1.5 flex flex-col overflow-hidden group cursor-pointer"
          ...
          >
            ...
            <div class="relative aspect-[4/3] overflow-hidden bg-slate-100">
              ...
              <!-- Price Pill -->
              <div class="absolute top-4 left-4 bg-slate-950/85 backdrop-blur-md text-white text-xs font-semibold px-3.5 py-1.5 rounded-lg flex items-center gap-1 shadow-md">
              ...
              <button 
                ...
                class="absolute top-4 right-4 w-9 h-9 rounded-lg bg-white/90 backdrop-blur-md flex items-center justify-center text-slate-500 hover:text-red-500 hover:scale-110 transition-all duration-200 shadow-sm"
              >
                <i class="pi pi-heart text-sm"></i>
              </button>

              <!-- Verified Pill -->
              <div class="absolute bottom-4 left-4 bg-emerald-500/90 backdrop-blur-md text-white text-[10px] font-semibold px-2.5 py-1 rounded-md flex items-center gap-1">
            ...
            <div class="p-6 flex flex-col flex-1 gap-3">
              <div class="flex items-center justify-between">
                <span class="text-xs font-semibold text-accent uppercase tracking-wider flex items-center gap-1">
                  <i class="pi pi-map-marker"></i> {{ apartment.city }}
                </span>
                <span class="text-xs font-semibold text-slate-700 flex items-center gap-1 bg-amber-50 px-2 py-0.5 rounded border border-amber-200">
                  <i class="pi pi-star-fill text-amber-500 text-[10px]"></i> 4.92
                </span>
              </div>

              <h3 class="text-lg font-semibold text-slate-900 group-hover:text-accent transition-colors line-clamp-1">
                {{ apartment.title }}
              </h3>
              ...
              <!-- Amenities Bar -->
              <div class="flex items-center gap-4 py-2.5 px-3 bg-slate-50 rounded-lg text-xs font-semibold text-slate-600 mt-auto">
              ...
              <div class="pt-3 border-t border-surface-border flex items-center justify-between">
                <span class="text-xs font-semibold text-slate-400">Instant Book</span>
                <span class="text-xs font-semibold text-accent group-hover:translate-x-1 transition-transform inline-flex items-center gap-1">
        ...
        <div v-else class="text-center py-16 bg-slate-50 rounded-xl border border-surface-border">
          <i class="pi pi-building text-4xl text-slate-300 mb-3 block"></i>
          <p class="font-semibold text-slate-700">No properties available in this category currently.</p>
    ...
    <section class="max-w-content mx-auto px-4 sm:px-6 py-12 sm:py-20">
      <div class="bg-gradient-to-br from-slate-900 via-navy-700 to-slate-900 rounded-2xl p-5 sm:p-12 lg:p-16 text-white relative overflow-hidden shadow-lg">
        <!-- Ambient Glow -->
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center relative z-10">
          ...
          <div class="lg:col-span-6 flex flex-col gap-6">
            <div>
              <span class="text-xs font-semibold uppercase tracking-widest text-accent mb-2 block">Interactive Vacation Planner</span>
              <h2 class="text-2xl sm:text-4xl font-semibold text-white tracking-tight leading-tight">
            ...
            <!-- Night Slider -->
            <div class="bg-white/10 backdrop-blur-md rounded-xl p-5 sm:p-6 border border-white/10">
              <div class="flex justify-between items-center mb-3">
                <label class="text-xs font-semibold text-slate-300 uppercase tracking-wider">Number of Nights</label>
                <span class="text-xl font-semibold text-amber-400">{{ calcNights }} {{ calcNights === 1 ? 'Night' : 'Nights' }}</span>
              </div>
              <input 
                type="range" 
                min="1" 
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3">
              <button
                ...
                class="p-3.5 rounded-lg text-left border transition-all cursor-pointer"
                :class="calcGrade === grade.id ? 'bg-accent border-accent text-white shadow-md' : 'bg-white/5 border-white/10 text-slate-300 hover:bg-white/10'"
              >
                <p class="text-xs font-semibold">{{ grade.label }}</p>
          ...
          <div class="lg:col-span-6">
            <div class="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl p-5 sm:p-8 relative shadow-lg">
              <div class="flex items-center justify-between border-b border-white/10 pb-6 mb-6">
                <div>
                  <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Estimated Total</p>
                  <p class="text-4xl font-semibold text-white mt-1">${{ calculatedTotal }} <span class="text-xs font-medium text-slate-400">USD</span></p>
                </div>
                <div class="text-right">
                  <p class="text-xs font-semibold text-accent uppercase tracking-wider">Apartex Loyalty Points</p>
                  <p class="text-3xl font-semibold text-amber-400 mt-1">+{{ calculatedPoints }} <span class="text-xs font-medium text-slate-400">PTS</span></p>
                </div>
              </div>

              <!-- Unlocked Perks -->
              <h4 class="text-xs font-semibold text-slate-300 uppercase tracking-widest mb-3">Unlocked Benefits For This Stay:</h4>
              <ul class="flex flex-col gap-3 mb-8">
                ...
                <li class="flex items-center gap-3 text-sm text-slate-200" v-if="calcNights >= 5">
                  ...
                  <span class="text-amber-300 font-semibold">Bonus: Free VIP Airport Shuttle Included</span>
                </li>
              </ul>

              <router-link 
                to="/apartments" 
                class="w-full btn-accent text-center font-semibold py-3.5 rounded-lg block no-underline shadow-accent hover:scale-[1.02] transition-transform"
              >
    ...
    <section class="max-w-content mx-auto px-4 sm:px-6 py-16">
      ...
      <div class="flex justify-center items-center gap-2 mb-10 overflow-x-auto pb-2 no-scrollbar">
        <button
          ...
          class="px-6 py-2.5 rounded-lg text-xs font-semibold transition-all cursor-pointer border"
      ...
      <div class="max-w-4xl mx-auto bg-white rounded-2xl border border-surface-border shadow-md p-8 sm:p-12 grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
        <!-- Digital Card Mockup -->
        <div 
          class="rounded-xl p-8 text-white relative overflow-hidden shadow-lg aspect-[1.6/1] flex flex-col justify-between transition-all duration-500"
          :class="activeTierData.cardBg"
        >
          <div class="flex justify-between items-start">
            <div>
              <p class="text-[10px] font-semibold uppercase tracking-widest opacity-75">Apartex Club VIP</p>
              <p class="text-2xl font-semibold tracking-wider mt-1">{{ activeTierData.name }} MEMBER</p>
            </div>
          ...
          <div>
            ...
            <p class="text-3xl font-semibold text-amber-300">{{ activeTierData.multiplier }}</p>
          </div>
          ...
          <div class="flex justify-between items-end border-t border-white/20 pt-3">
            <div>
              ...
              <p class="text-sm font-semibold">{{ activeTierData.minNights }} Nights / Year</p>
            </div>
            <span class="text-xs font-semibold px-3 py-1 bg-white/20 rounded-md backdrop-blur-md">APARTEX VIP</span>
          </div>
        </div>

        <!-- Tier Privileges -->
        <div class="flex flex-col gap-4">
          <span class="text-xs font-semibold text-accent uppercase tracking-widest">{{ activeTierData.name }} Privileges</span>
          <h3 class="text-2xl font-semibold text-slate-900">Why Join {{ activeTierData.name }} Tier?</h3>
          ...
          <router-link to="/loyalty" class="inline-flex items-center gap-2 text-sm font-semibold text-accent hover:text-accent-hover mt-2 no-underline">
      ...
      <div class="max-w-content mx-auto px-4 sm:px-6">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div class="lg:col-span-6 flex flex-col gap-4">
            ...
            <router-link to="/register?role=owner" class="btn-accent text-center sm:w-fit px-8 py-3.5 text-sm font-semibold no-underline shadow-md">
              Become a Host Today
            </router-link>
          </div>
          ...
          <div class="lg:col-span-6">
            <div class="bg-white rounded-2xl border border-surface-border p-5 sm:p-8 shadow-md">
              ...
              <!-- City Selection -->
              <div class="mb-5">
                <label class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 block">Property City</label>
              ...
              <div class="mb-6">
                <div class="flex justify-between items-center mb-2">
                  <label class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Number of Bedrooms</label>
                  <span class="text-sm font-semibold text-navy">{{ hostBedrooms }} Bedrooms</span>
                </div>
                <input type="range" min="1" max="5" v-model.number="hostBedrooms" class="w-full accent-accent" />
              </div>

              <!-- Monthly Result Box -->
              <div class="bg-accent-light rounded-xl p-6 border border-orange-200 text-center">
                <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Estimated Monthly Earnings</p>
                <p class="text-4xl font-semibold text-accent my-2">${{ estimatedHostIncome }} <span class="text-xs text-slate-500 font-normal">/ month</span></p>
                <p class="text-xs text-slate-500">Based on an average 72% monthly occupancy rate in {{ hostCity }}.</p>
              </div>

              <router-link to="/register?role=owner" class="w-full mt-6 bg-navy text-white text-center font-semibold text-sm py-3.5 rounded-lg block no-underline hover:bg-slate-800 transition-colors">
    ...
    <section class="max-w-content mx-auto px-4 sm:px-6 py-20">
      <div class="text-center max-w-2xl mx-auto mb-14 reveal-up">
        ...
        <div class="flex items-center justify-center gap-2 mt-3">
          ...
          <span class="text-sm font-semibold text-slate-800">4.9 / 5.0 Rating</span>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8 reveal-up delay-100">
        <div 
          ...
          :key="review.author" 
          class="bg-white rounded-xl border border-surface-border p-7 shadow-sm hover:shadow-md transition-all flex flex-col justify-between"
        >
          <div class="flex flex-col gap-3">
            <div class="flex justify-between items-center">
              ...
              <span class="text-[10px] font-semibold text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-md border border-emerald-200">
          ...
          <div class="flex items-center gap-3 pt-4 border-t border-slate-100 mt-6">
            <div class="w-10 h-10 rounded-full bg-navy text-white font-semibold text-sm flex items-center justify-center shrink-0 shadow-sm">
              {{ review.initials }}
            </div>
            <div>
              <p class="font-semibold text-sm text-slate-900">{{ review.author }}</p>
      ...
      <div class="max-w-3xl mx-auto px-4 sm:px-6">
        ...
        <div class="flex flex-col gap-4">
          <div
            ...
            :key="index"
            class="border border-surface-border rounded-xl overflow-hidden transition-all"
            :class="openFaqIndex === index ? 'bg-slate-50 border-slate-300 shadow-sm' : 'bg-white'"
          >
            <button
              @click="openFaqIndex = openFaqIndex === index ? -1 : index"
              class="w-full text-left p-6 font-semibold text-slate-900 text-base flex justify-between items-center gap-4 cursor-pointer"
            >
    ...
    <footer class="bg-slate-950 text-white border-t border-slate-800">
      ...
      <div class="border-b border-slate-800 py-12">
        <div class="max-w-content mx-auto px-4 sm:px-6 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          <div class="lg:col-span-7">
            <h3 class="text-2xl font-semibold text-white">Join the Apartex VIP Travelers Club</h3>
          ...
          <div class="lg:col-span-5 flex gap-2">
            ...
            />
            <button @click="subscribeNewsletter" class="btn-accent shrink-0 font-semibold px-6">
              Subscribe
            </button>
          </div>
        </div>
      ...
      <div class="max-w-content mx-auto px-4 sm:px-6 py-16 grid grid-cols-1 md:grid-cols-4 gap-10">
        <div class="col-span-1">
          <div class="flex items-center gap-2 text-xl font-semibold tracking-tight text-white mb-4">
          ...
          <div class="flex gap-3">
            <a href="#" class="w-9 h-9 rounded-lg border border-slate-800 flex items-center justify-center text-slate-400 hover:border-accent hover:text-accent transition-colors"><i class="pi pi-facebook text-sm"></i></a>
            <a href="#" class="w-9 h-9 rounded-lg border border-slate-800 flex items-center justify-center text-slate-400 hover:border-accent hover:text-accent transition-colors"><i class="pi pi-twitter text-sm"></i></a>
            <a href="#" class="w-9 h-9 rounded-lg border border-slate-800 flex items-center justify-center text-slate-400 hover:border-accent hover:text-accent transition-colors"><i class="pi pi-instagram text-sm"></i></a>
          </div>
        </div>

        <div>
          <h4 class="text-xs font-semibold uppercase tracking-widest text-slate-400 mb-4">Quick Links</h4>
        ...
        <div>
          <h4 class="text-xs font-semibold uppercase tracking-widest text-slate-400 mb-4">Top Destinations</h4>
          <ul class="flex flex-col gap-2.5 text-sm text-slate-400">
        ...
        <div>
          <h4 class="text-xs font-semibold uppercase tracking-widest text-slate-400 mb-4">Accepted Payments</h4>
